...
const LogInRecipient = (props) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post('http://localhost:5000/api/auth/recipient/login', post)
            if (res.data.user) {
        ...
        <div className="auth-form-container" style={{padding: 100}}>
            ...
            <form className="login-form" onSubmit={handleSubmit}>
                ...
                <button type="submit" style={{
                    marginTop: 20, borderRadius : 10}}>Log In As Recipient</button>
            </form>
            </div>
            <ToastContainer />
        </>
